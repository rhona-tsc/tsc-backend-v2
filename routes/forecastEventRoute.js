...
const parseCsvDate = (v) => {
  ...
  if (dmy) {
    return new Date(
      `${dmy[3]}-${dmy[2].padStart(2, "0")}-${dmy[1].padStart(2, "0")}T00:00:00.000Z`,
    );
  }
...
forecastEventRouter.post(
  "/bulk-supplier-payouts-csv",

  express.text({ type: ["text/*", "text/csv", "application/csv"] }),

  async (req, res) => {
    try {
      const csv = typeof req.body === "string" ? req.body : req.body?.csv;

      const entity = req.query.entity || req.body?.entity || "BMM";

      const replaceExisting =
        String(
          req.query.replaceExisting || req.body?.replaceExisting || "false",
        ) === "true";
      ...
      if (!csv || typeof csv !== "string") {
        return res.status(400).json({
          ...
          message: "CSV string is required",
        });
      }
      ...
      for (const [index, row] of records.entries()) {
        try {
          ...
          if (!expectedDate)
            throw new Error("Missing/invalid Date of transaction");
          ...
          docs.push({
            ...
            notes: [
              ...
              eventDate
                ? `Event date: ${eventDate.toISOString().slice(0, 10)}`
                : "",
