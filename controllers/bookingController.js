...
// ---- V2 mailer used by completeBookingV2 ----
async function sendContractEmail({ booking, pdfBuffer }) {
  console.log("📧 [Email V2] Preparing contract email...", {
    bookingId: booking?.bookingId,
    to: booking?.userAddress?.email || booking?.userEmail || null,
  });

  const to = booking?.userAddress?.email || booking?.userEmail || null;
  if (!to) {
    console.error("❌ [Email V2] No client email on booking — aborting send.");
    throw new Error("Missing client email");
  }

  // Build a transporter fresh each send (isolated from other flows)
  const transporter = nodemailer.createTransport({
    host: process.env.SMTP_HOST,
    port: Number(process.env.SMTP_PORT || 587),
    secure: false,
  ...
  const eventDate = new Date(booking?.date || booking?.eventDate || Date.now());
  const fmt = (d) =>
  ...
  // Keep body content close to V1 for continuity (no external `signature` var)
  const bodyHtml = `
  ...
  // Try to inline our signature if helper exists; otherwise skip quietly
  const attachments = [
    { filename: `Booking_${booking.bookingId}.pdf`, content: pdfBuffer },
  ];
  try {
    if (typeof resolveSignatureGifPath === "function") {
      const sigPath = resolveSignatureGifPath();
      if (sigPath) {
        attachments.push({
          filename: "signature.gif",
          path: sigPath,
          cid: "signature.gif", // only used if body references it
  ...
  try {
    const info = await transporter.sendMail(mailOptions);
    console.log("✅ [Email V2] Contract email dispatched", {
    ...
    console.error("❌ [Email V2] Nodemailer send failed:", err?.message || err);
    throw err;
  }
}
